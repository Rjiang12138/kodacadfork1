...
class DocModel:
    ...
    def parse_doc(self):
        ...
        # 1) 用 free shapes 获取 root（更符合 STEP 导入语义）
        labels = TDF_LabelSequence()
        shape_tool.GetFreeShapes(labels)
        nbr = labels.Length()
        logger.debug(f"Number of free shapes at doc root: {nbr}")

        if nbr < 1:
            print("Empty document (no free shapes).")
            return

        # 2) 逐个处理每个 free shape（不要只取 Value(1)）
        for i in range(1, nbr + 1):
            ...
            self.label_dict[root_uid] = {
                "entry": root_entry,
                "name": root_name,
                "parent_uid": None,
                "ref_entry": None,
                "is_assy": shape_tool.IsAssembly(root_label),
                "inv_loc": loc.Inverted(),
            }

            # 3) 如果是装配：按原逻辑解析 components
            if shape_tool.IsAssembly(root_label):
                top_comps = TDF_LabelSequence()
                subchilds = False
                shape_tool.GetComponents(root_label, top_comps, subchilds)
                if top_comps.Length():
                    ...
                    # 装配但没有组件：不算“出错”，只是不包含子项
                    logger.debug(f"Assembly root has no components: {root_entry}")

            # 4) 如果是简单形状：直接作为 part 放进 part_dict
            else:
                shape = shape_tool.GetShape(root_label)
                color = Quantity_Color()
                color_tool.GetColor(shape, XCAFDoc_ColorSurf, color)

                # 作为“可显示零件”放入 part_dict（这里 key 用 root_uid）
                self.part_dict[root_uid] = {
                    "shape": shape,
                    "color": color,
                    "name": root_name,
    ...
    def save_step_doc(self):
        ...
        if status != IFSelect_RetDone:
            print(f"Step file {fname} was not saved.because {status}")
    ...
    def add_component_to_asy(self, shape, name, color, tag=1):
        ...
        if labels.Length() == 0:
            ...
            top_shape = TopoDS_Compound()
            bld = BRep_Builder()
            bld.MakeCompound(top_shape)

            top_label = shape_tool.AddShape(top_shape, True)  # True => as assembly
            set_label_name(top_label, "Top")
            shape_tool.UpdateAssemblies()
        ...
        if tag < 1 or tag > labels.Length():
            print(f"add_component_to_asy(): invalid tag={tag}, root labels={labels.Length()}")
            return None
        try:
            asyLabel = labels.Value(tag)  # label at root with tag
        except RuntimeError as e:
            print(e,"component failed")
            return
